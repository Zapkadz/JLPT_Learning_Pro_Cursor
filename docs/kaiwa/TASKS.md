...
| KAI-024 · M · Speech/Backend | Quality gate audio: silence/clipping/noise/reference leakage và vùng không chấm | KAI-020, KAI-023 | Test im lặng/loa phát mẫu/nhiễu/chồng giọng; tình huống không đủ tin cậy trả unavailable/reason, không biến thành điểm phát âm 0; không coi phát mẫu qua loa là người học hoàn thành | DONE (leakage heuristic; WebM decode deferred) |
...
```text
Task: KAI-024 leakage DONE. Ops blockers: live PA sample / corpus / KAI-076.
WebM PCM decode for quality+leakage on real mic still deferred.
```
...
| --- | --- | --- | --- |
| 18/09/2026 | KAI-024 reference leakage NCC (optional referencePcm) | `npm test` **151/151** | Wire decoded ref in API; ops inputs |
| 18/09/2026 | KAI-026 pronunciation live-check harness (no invented API JSON) | `npm test` **148/148** | Live sample or corpus |
